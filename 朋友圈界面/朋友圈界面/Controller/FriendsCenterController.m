...
- (void)viewDidLoad {
    ...
    NSArray *arr=@[@"刘成成",@"韩李涛",@"张衡",@"张衡"];
    NSArray *arr1=@[@"咦嘘兮，危乎高哉，蜀道之难，难于上青天",@"不知天上宫阙，今昔是何年",@"王国维谈治学三境界说：“古今之成大事业、大学问者,必经过三种之境界：‘昨夜西风凋碧树.独上高楼,望尽天涯路’.此第一境也.‘衣带渐宽终不悔,为伊消得人憔悴.’此第二境也.‘众里寻他千百度,蓦然回首,那人却在,灯火阑珊处’.此第三境也",@"咦嘘兮，危乎高哉，蜀道之难，难于上青天"];
...
- (void)SetupTableview{
    _tableview=[[HLTTableview alloc]initWithFrame:CGRectMake(0, 64, 375, 667-64) style:UITableViewStylePlain];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.commentBtn addTarget:self action:@selector(begincomment:) forControlEvents:UIControlEventTouchUpInside];
     [cell.priaseBtn addTarget:self action:@selector(praise:) forControlEvents:UIControlEventTouchUpInside];
    return cell;//将设置好的cell返回
}
#pragma mark-开始评论
- (void)begincomment:(UIButton*)sender{
    self.needUpdateOffset = YES;
    self.history_Y_offset = [sender    convertRect:sender.bounds toView:[UIApplication sharedApplication].keyWindow].origin.y;
...
- (void)answer:(UITableView *)tableview indexPath:(NSIndexPath *)indexPath{
    ...
    self.seletedCellHeight=cell.frame.size.height;
     self.needUpdateOffset = YES;
    self.history_Y_offset = [cell convertRect:cell.bounds toView:[UIApplication sharedApplication].keyWindow].origin.y;
...
{ NSDictionary *userInfo = [notification userInfo];
    ...
    if (self.messagetype==Answermessage){//点击某行，进行回复某人
        delta = self.history_Y_offset - ([UIApplication sharedApplication].keyWindow.bounds.size.height - keyboardHeight-  self.seletedCellHeight-self.commentview.frame.size.height);
    }else{//点击评论按钮
      delta = self.history_Y_offset - ([UIApplication sharedApplication].keyWindow.bounds.size.height - keyboardHeight-  self.commentview.frame.size.height-30-2);//
    }
